...
const GPayModal = ({ category, isOpen, onClose, onSuccess }) => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // 1. Create intent / order
      const { data: intentData } = await api.post('/payments/create-intent', {
        amount: numAmount,
        categoryId: category._id
      });
      
      const refId = intentData.data.referenceId;
      setReferenceId(refId);

      // 2. Trigger GPay / UPI intent
      const upiUrl = `upi://pay?pa=aathi1234@oksbi&pn=FinanceTracker&am=${numAmount}&cu=INR&tr=${refId}`;
          ...
          <>
            ...
            <div className="mb-6">
              <p className="text-[#94A3B8] mb-4 text-sm leading-relaxed">
                Please complete the payment of <strong className="text-white">{currencySymbol}{amount}</strong> in your UPI app.
              </p>
              <p className="text-[#94A3B8] text-sm leading-relaxed">
                Once the payment is successful, click the button below to record the transaction.
              </p>
            </div>
